...
class BaseAction():
    ...
    def find_toast(self, message, is_screenshot = False, screenshot_name = None, timeout=3, poll=0.1):
        ...
        '''
        message = "//*[contains(@text,'" + message + "')]"  # 使用包含方式定位
        try:
    ...
    def is_toast_exist(self, message, is_screenshot = False, screenshot_name = None):
        try:
            self.find_toast(message, is_screenshot, screenshot_name)
            return True
        except Exception:
            return False
